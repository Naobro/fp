...
if st.button("PDFダウンロード"):
    with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as tmp:
        ...
        pdf.add_font('NotoSansJP', '', FONT_REGULAR, uni=True)
        pdf.add_font('NotoSansJP', 'B', FONT_BOLD, uni=True)
        ...
        pdf.set_font('NotoSansJP', 'B', 11.5)
        pdf.cell(0, 7, f"物件名：{property_name}", ln=1, align='L')
        pdf.set_font('NotoSansJP', 'B', 13)
        ...
        # --- 諸費用合計・総合計（横並び/190mm幅を完全に使用） ---
        pdf.set_fill_color(220, 240, 255)
        sum_cols = [45, 50, 55, 40]  # 合計で190
        # ↓両方とも同じフォント・同じ太字
        pdf.set_font('NotoSansJP', 'B', 12)
