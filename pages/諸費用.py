# fp/pages/諸費用.py  —— 完全自給・utils非依存・日本語PDF対応
...
# ========== フォントの絶対パス ==========
# このファイルは fp/pages/ にある想定。parents[1] が fp/
FP_DIR = Path(__file__).resolve().parents[1]
FONT_DIR = FP_DIR / "fonts"
FONT_REGULAR = FONT_DIR / "NotoSansJP-Regular.ttf"
FONT_BOLD    = FONT_DIR / "NotoSansJP-Bold.ttf"

def _assert_fonts():
    miss = [p for p in (FONT_REGULAR, FONT_BOLD) if not p.exists()]
    if miss:
        tried = "\n".join(f" - {p}" for p in (FONT_REGULAR, FONT_BOLD))
        st.error("日本語フォントが見つかりません。次のパスに置いてください。")
        st.code(tried)
        raise FileNotFoundError("NotoSansJP ttf が見つかりません。")
...
if st.button("PDFダウンロード"):
    ...
    try:
        ...
        # フォント登録を「add_page() より先」にやる（header等で使う可能性があるため）
