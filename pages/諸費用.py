...
def build_pdf():
    ...
    money_rows = [
        ...
        ["決済時必要自己資金", fmt_jpy(settlement_self_funds), f"売主支払額{fmt_jpy(seller_payment)}＋住宅ローン手数料{fmt_jpy(loan_fee)}＋仲介手数料 決済時金額{fmt_jpy(broker_settlement)}－借入金額{fmt_jpy(loan_amount)}"],
    ]
    ...
    for r in money_rows:
        ...
        line_count = max(
            1,
            len(str(r[2]).split("\n")) + int(pdf.get_string_width(str(r[2])) / 42) if str(r[2]) else 1
        )
        row_h = max(7, 6 * line_count)
        ...
        pdf.multi_cell(money_w[0], 6, str(r[0]), border=0)

        pdf.set_xy(x_row + money_w[0], y_row)
        pdf.cell(money_w[1], row_h, str(r[1]), border=0, align="R")

        pdf.set_xy(x_row + money_w[0] + money_w[1], y_row)
        pdf.multi_cell(money_w[2], 6, str(r[2]), border=0)
    ...
    def draw_table(title, rows):
        ...
        for r in rows:
            ...
            line_count = max(
                1,
                len(desc_text.split("\n")) + int(pdf.get_string_width(desc_text) / 30)
            )
            row_h = max(6, 6 * line_count)
            ...
            pdf.set_xy(x_row, y_row)
            pdf.multi_cell(w[0], 6, str(r[0]), border=0)

            pdf.set_xy(x_row + w[0], y_row)
            pdf.cell(w[1], row_h, str(r[1]), border=0, align="R")

            pdf.set_xy(x_row + w[0] + w[1], y_row)
            pdf.cell(w[2], row_h, str(r[2]), border=0, align="C")

            pdf.set_xy(x_row + w[0] + w[1] + w[2], y_row)
            pdf.multi_cell(w[3], 6, desc_text, border=0)
