# fp/pages/諸費用.py
# 改訂版（2025-10-09）
# 手付金×5%自動・借入金×2.2%自動・仲介料自動分配・PDF＆Supabase完全版
...
def save_to_state(k, v): st.session_state[k] = v; return v
...
def _download_and_extract_ttf(url, suffix, save_path):
    resp = requests.get(url, timeout=30)
    resp.raise_for_status()
    with zipfile.ZipFile(io.BytesIO(resp.content)) as zf:
        ttf = [n for n in zf.namelist() if n.lower().endswith(suffix)][0]
        with zf.open(ttf) as src, open(save_path, "wb") as dst: dst.write(src.read())
...
with col2:
    is_new = st.checkbox("新築戸建（表示登記あり）", value=st.session_state.get("is_new", False))
with col3:
    use_flat35 = st.checkbox("フラット35（適合証明）", value=st.session_state.get("use_flat35", False))
...
# ========== 物件価格・借入金額 ==========
price_man = st.number_input("物件価格（万円）", min_value=100, max_value=200_000, value=st.session_state.get("price_man", 5800), step=10)
save_to_state("price_man", price_man)
property_price = price_man * 10_000

# 手付金（自動5%・50万単位丸め）
deposit_default = round_deposit(property_price)
deposit = number_input_commas("手付金（円）", st.session_state.get("deposit", deposit_default))
save_to_state("deposit", deposit)

# 借入金額（万円単位）
loan_amount_man = st.number_input("借入金額（万円）", min_value=0, max_value=200_000, value=st.session_state.get("loan_amount_man", int(price_man)), step=10)
loan_amount = loan_amount_man * 10_000
save_to_state("loan_amount", loan_amount)

# 銀行事務手数料（自動：借入金額×2.2%）
loan_fee_auto = int(loan_amount * 0.022)
loan_fee = number_input_commas("銀行事務手数料（円）", st.session_state.get("loan_fee", loan_fee_auto))
save_to_state("loan_fee", loan_fee)

# 印紙税
elec_contract = st.checkbox("電子契約（印紙代 0円）", value=st.session_state.get("elec_contract", False))
...
display_fee = number_input_commas("表示登記（円）", st.session_state.get("display_fee", 110_000 if (prop_type=="戸建て" and is_new) else 0))
tekigo_fee = number_input_commas("適合証明書（円）", st.session_state.get("tekigo_fee", 55_000 if use_flat35 else 0))
reform_fee = number_input_commas("追加リフォーム（円）", st.session_state.get("reform_fee", 0))
move_fee = number_input_commas("引越し費用（円）", st.session_state.get("move_fee", 120_000))

for k,v in [("regist_fee",regist_fee),("fire_fee",fire_fee),("tax_clear",tax_clear),
            ("display_fee",display_fee),("tekigo_fee",tekigo_fee),
            ("reform_fee",reform_fee),("move_fee",move_fee)]:
    save_to_state(k,v)
...
def build_pdf():
    ...
    pdf.cell(0,6,f"物件名：{st.session_state['property_name']}",ln=1)
    pdf.cell(0,6,f"物件価格：{fmt_jpy(property_price)}",ln=1)
    pdf.cell(0,6,f"手付金：{fmt_jpy(deposit)}（物件価格の5%目安）",ln=1)
    pdf.ln(4)
    ...
    def draw_table(title,rows):
        ...
        for h,ww in zip(["項目","金額","支払時期","説明"],w):
    ...
    draw_table("◆ 登記・税金・精算金",[
        ...
        ["精算金",fmt_jpy(tax_clear),"決済時","固都税・管理費等日割"],
        ["表示登記",fmt_jpy(display_fee),"決済時","新築戸建で必要（目安10万）"]
    ])
    draw_table("◆ 金融機関・火災保険",[
        ["銀行事務手数料",fmt_jpy(loan_fee),"決済時","借入金額×2.2%"],
        ["火災保険料",fmt_jpy(fire_fee),"決済時","5年契約想定"],
        ["適合証明書",fmt_jpy(tekigo_fee),"相談","フラット35時に必要"]
    ])
...
st.download_button("📄 PDFダウンロード", data=pdf_bytes,
    file_name=f"{st.session_state['property_name']} 諸費用明細.pdf", mime="application/pdf")
