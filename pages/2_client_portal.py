...
# クエリ/トークンは完全撤廃。固定 CLIENT_ID のみで開く。
...
with st.form("hearing_form", clear_on_submit=False):
    ...
    with c3:
        hearing["now_rent"]  = st.number_input("住居費（万円/月）", min_value=0, max_value=200, value=int(hearing.get("now_rent",10)))
...
if submitted:
    ...
    try:
        ...
        pair("住居費（万円/月）", hearing["now_rent"]); pair("ご家族構成", hearing["family"])
...
if "baseline" not in payload:
    payload["baseline"] = {
        "housing_cost_m": int(hearing.get("now_rent", 10)),
        "walk_min": 10,
        "area_m2": 60,
        "floor": 3,
