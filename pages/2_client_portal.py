...
# 偏差値（線形）ヘルパー：平均3.0→50、差分×10でスケール
def to_hensachi_from_diff(avg: float, base_avg: float) -> float:
    return round(50 + (avg - base_avg) * 10, 1)
...
def render_client_portal(client_id: str):
    ...
    # ============================================
    # ⑤ 物件比較（別ページへ：ランチャー固定URL）
    # ============================================
    st.header("⑤ 物件比較（別ページ）")
    st.caption("このページではヒアリング＆プロフィールを整えます。比較は専用URLで横並び入力・偏差値表示（現住居=50）。")

    compare_launcher_path = f"pages/compare/{client_id}.py"
    btn_label = "↔ 物件比較ページを開く"
    try:
        st.page_link(compare_launcher_path, label=btn_label, icon="↔️")
    except Exception:
        st.markdown(f"**{btn_label}** をサイドバーから選択してください。")
