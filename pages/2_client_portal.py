...
def load_client_by_token(token: str):
    # data/clients/*.json を舐めて meta.token が一致するものを返す
    for path in DATA_DIR.glob("*.json"):
        try:
            obj = json.loads(path.read_text(encoding="utf-8"))
        except Exception:
            continue
        if obj.get("meta", {}).get("token") == token:
...
# ================
q = st.query_params
token_in = q.get("token") or q.get("t")
client_id = None
payload = None

if token_in:
    client_id, payload = load_client_by_token(token_in)
else:
    # 後方互換：?client=
    client_in = q.get("client")
    if client_in:
        client_id = client_in
        payload = load_client_by_id(client_in)
...
client_name = meta.get("name", "お客様")
token_in = meta.get("token", token_in)  # ファイル側に保存されている token を正とする

st.title("理想の住まいへのロードマップ")
st.success(f"{client_name} 専用ページ")
...
# ★ base_defaults を5カテゴリに統一（価格/立地/広さ・間取り/スペック/管理・共有部・その他）
base_defaults = {
    # 表示名
    "name": meta.get("name", ""),
    # 現状ベース
    "now_area": "", "now_years": 5, "is_owner": "賃貸", "now_rent": 10, "family": "",
    # 満足・不満
    "sat_point": "", "sat_price": 3, "sat_location": 3, "sat_size": 3, "sat_spec": 3, "sat_age": 3,
    "dissat_free": "",
    # 収入・勤務（任意）
...
if "hearing_data" not in st.session_state:
    st.session_state["hearing_data"] = {**base_defaults, **payload.get("hearing", {})}
else:
    # 欠損を補完（新キーが増えても落ちない）
...
with st.form("hearing_form", clear_on_submit=False):
    ...
    with sc4: hearing["sat_spec"] = st.slider("満足度：スペック", 1, 5, int(hearing["sat_spec"]))
    with sc5: hearing["sat_age"] = st.slider("満足度：築年数（参考）", 1, 5, int(hearing["sat_age"]))
...
if submitted:
    ...
    try:
        ...
        def pair(label, val):
            ...
            pdf.set_font("NotoSansJP","",11); txt = "（未入力）" if val in [None, ""] else str(val)
        ...
        title("5W2H（購入計画）")
        for lbl, key in [("Why", "w_why"),("When","w_when"),("Where","w_where"),("Who","w_who"),
                         ("What","w_what"),("How","w_how"),("How much","w_howmuch"),("補足","w_free")]:
            pair(lbl, hearing[key])

        title("重要度のトレードオフ")
        pair("価格/立地/広さ間取り/スペック/管理",
...
with st.expander("価格（重視度のみ）", expanded=False):
    wish_select("価格の重視度", "imp_price")
...
with st.expander("広さ・間取り", expanded=False):
    for key, label in [
        ("sz_area","専有面積の広さ"),
        ("sz_living","リビングの広さ"),
        ("sz_layout","優れた間取り（ワイドスパン等）"),
        ("sz_storage","収納量（WIC/SIC等）の充実"),
        ("sz_ceiling","天井高が高い"),
        ("sz_aspect","日当たり（向き）の良さ"),
        ("sz_balcony_depth","バルコニー奥行の余裕"),
        ("sz_sun_wind","採光・通風の良さ"),
        ("sz_flow","廊下幅・家事動線の良さ"),
    ]:
        wish_select(label, key)
...
with st.expander("スペック（専有部分）", expanded=False):
    ...
    # 暖房・空調
    for key, label in [("h_floorheat","床暖房"), ("h_aircon_built","エアコン（備付）")]:
        wish_select(label, key)
    # 窓・建具
    for key, label in [
...
with st.expander("管理・共有部・その他", expanded=False):
    for key, label in [
        ...
        ("c_pet_ok","ペット可"), ("c_ltp_plan","長期修繕/資金計画の良さ"),
...
# ============================================
# ⑤ 物件比較（別ページへ）— token で渡す
# ============================================
st.header("⑤ 物件比較（別ページ）")
st.caption("このページでは“プロフィール”を整えます。比較は別ページで、複数物件を横並びで比較できます。")

link_label = "↔ 物件比較ページを開く"
# 比較ページのファイル名は、ユーザー提示の現行コード名に合わせる
compare_page = "3_compare.py"  # 例：pages/ 配下ではなくルートにある想定

# Streamlitのルーティングに依存せずリンク文字列で誘導
if token_in:
    st.markdown(f"[{link_label}](./{compare_page}?token={token_in})")
else:
    st.markdown(f"[{link_label}](./{compare_page}?client={client_id})")