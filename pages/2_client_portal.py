...
# PIN が付与されている場合のみ照合
save_pin = payload.get("meta", {}).get("pin")
if pin_in and save_pin and pin_in != save_pin:
...
with st.form("hearing_form", clear_on_submit=False):
    ...
    with p3: hearing["prio_size"] = st.selectbox("広さ", [1,2,3,4,5], index=int(hearing["prio_size"])-1)
    with p4: hearing["prio_age"] = st.selectbox("築年数（参考）", [1,2,3,4,5], index=int(hearing["prio_age"])-1)
    with p5: hearing["prio_spec"] = st.selectbox("スペック", [1,2,3,4,5], index=int(hearing["prio_spec"])-1)
...
# ============================================
st.header("⑤ 物件比較（別ページ）")
note = "このページでは“プロフィール”を整えます。比較自体は別ページで、複数物件を横並びで比較できます。"
st.caption(note)

# ナビゲーション（Streamlit 1.30+ の st.page_link があれば使う）
link_label = "↔ 物件比較ページを開く"
try:
    st.page_link("pages/3_compare_properties.py", label=link_label, icon="↔️")
except Exception:
    # 直接URLリンク（相対パス）。デプロイ先によっては無視されるため併記。
    base = "pages/3_compare_properties.py"
    if pin_in:
        st.markdown(f"[{link_label}](./{base}?client={client_id}&pin={pin_in})")
    else:
        st.markdown(f"[{link_label}](./{base}?client={client_id})")