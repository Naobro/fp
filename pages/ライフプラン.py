...
# 公式IPAex直リンク（単体zip）
_IPAEX_G_ZIP = "https://moji.or.jp/wp-content/ipafont/IPAexfont/ipaexg00401.zip"  # ゴシック
# （必要なら明朝も可） _IPAEX_M_ZIP = "https://moji.or.jp/wp-content/ipafont/IPAexfont/ipaexm00401.zip"

# 保存先（ユーザー毎のフォント置き場）
_FONT_DIR = Path.home() / ".matplotlib" / "fonts" / "ipaex"
_FONT_DIR.mkdir(parents=True, exist_ok=True)
_IPAEX_G_TTF = _FONT_DIR / "IPAexGothic.ttf"   # zip内 ipaexg.ttf をこの名前で保存

def _download_ipaex_gothic():
    resp = requests.get(_IPAEX_G_ZIP, timeout=30)
    resp.raise_for_status()
    with zipfile.ZipFile(io.BytesIO(resp.content)) as zf:
        # zip内の ipaexg.ttf を抽出
        members = [n for n in zf.namelist() if n.lower().endswith("ipaexg.ttf")]
        if not members:
            raise RuntimeError("ipaexg.ttf not found in zip")
        with zf.open(members[0]) as src, open(_IPAEX_G_TTF, "wb") as dst:
            dst.write(src.read())

def set_matplotlib_japanese_font():
    """IPAexゴシックをMatplotlibに登録し、日本語を確実表示"""
    if not _IPAEX_G_TTF.exists():
        _download_ipaex_gothic()
    # フォントマネージャに追加→キャッシュ更新→既定フォントに指定
    font_manager.fontManager.addfont(str(_IPAEX_G_TTF))
    # Matplotlib 3.6+ では rebuild は自動反映されるが明示しておく
    try:
        font_manager._load_fontmanager(try_read_cache=False)  # 内部APIだが実運用で安定
    except Exception:
        pass
