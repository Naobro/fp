...
if "compare_rows" not in st.session_state:
    st.session_state["compare_rows"] = 3  # デフォ3件
...
# --- ② 立地
st.markdown("### ② 立地（資産性）")
fields_loc = ["最寄徒歩（分）","複線数（本）","職場アクセス（分）","商業施設","教育","医療","治安","災害","公園・緑地","騒音"]
loc_inputs = []
for i in range(rows):
    with st.container():
...
st.divider()
st.caption("※ ロジックは暫定です。現場運用で係数・閾値は都度チューニング可能。")