...
# ポイント合計→金利引下げ表
# key: (min_point, max_point) → {当初5年: 引下, 6~10年: 引下, 11~15年: 引下}
DISCOUNT_TABLE = [
    (1, 1, {"5年":  0.25, "10年": 0.00, "15年": 0.00}),
    (2, 2, {"5年":  0.50, "10年": 0.00, "15年": 0.00}),
    (3, 3, {"5年":  0.75, "10年": 0.00, "15年": 0.00}),
    (4, 4, {"5年":  1.00, "10年": 0.00, "15年": 0.00}),
    (5, 5, {"5年":  0.25, "10年": 0.50, "15年": 0.00}),
    (6, 6, {"5年":  0.50, "10年": 1.00, "15年": 0.00}),
    (7, 7, {"5年":  0.75, "10年": 1.00, "15年": 0.00}),
    (8, 8, {"5年":  1.00, "10年": 1.00, "15年": 0.00}),
    (9, 99, {"5年": 0.25, "10年": 0.50, "15年": 1.00}),
]

def get_discount(total_p: int):
    for lo, hi, disc in DISCOUNT_TABLE:
        if lo <= total_p <= hi:
            return disc
    return {"5年": 0.0, "10年": 0.0, "15年": 0.0}
...
# ──────────────────────────────────────────────────────────────────────
def build_pdf(
    principal_man, years, loan_amount_man, jishikin_man, chokkin_rate,
    var_rate, flat_base, flat_total_p, flat_disc,
    ...
    disc = flat_disc
    cond_data += [
        ["引下げ（当初5年）", f"▲{disc['5年']:.2f} ％  →  適用金利 {max(flat_base - disc['5年'], 0):.3f} ％"],
        ["引下げ（6〜10年目）", f"▲{disc['10年']:.2f} ％  →  適用金利 {max(flat_base - disc['10年'], 0):.3f} ％"],
        ["引下げ（11〜15年目）", f"▲{disc['15年']:.2f} ％  →  適用金利 {max(flat_base - disc['15年'], 0):.3f} ％"],
    ]
    t = Table(cond_data, colWidths=[60*mm, 100*mm])
    ...
    for name, data in scenarios_data.items():
        ...
        rows_pdf.append(row)
    ncols = len(header)
    col_w = [38*mm] + [(186 - 38) / max(ncols - 1, 1) * mm] * (ncols - 1)
    ...
    inv_data = [
        ["項目", "金額"],
        ["変動返済額（月）", f"{monthly_payment(loan_amount_man*10000, var_rate, years*12):,.0f} 円"],
        ["フラット35返済額（月）", f"{monthly_payment(loan_amount_man*10000, flat_base, years*12):,.0f} 円"],
        ["差額（月）", f"{diff_monthly:,.0f} 円"],
        [f"差額を年利 {invest_rate:.1f}% で {invest_years} 年間運用した場合", f"{future_value/10000:,.1f} 万円"],
    ]
    t3 = Table(inv_data, colWidths=[100*mm, 60*mm])
    ...
    # ── メリット・デメリット ────────────────────────────────────
    story.append(Paragraph("■ 変動金利 vs 固定金利（フラット35）メリット・デメリット", jp_h2))
    merit_data = [
        ["", "変動金利", "フラット35（固定）"],
        ["メリット",
         "・借入時の金利が低く月々返済を抑えやすい\n・金利低下時に返済負担が軽減\n・団信の選択肢が多い",
        ...
        with cols_g[gi]:
            ...
            for label, pt in items.items():
                if pt is None:
                    # 子どもN人：手入力
                    n = st.number_input(
                        f"{label}", min_value=0, max_value=10, value=0, step=1,
                        key=f"flat35s_{gname}_{label}"
                    )
                    total_points += n
                else:
                    checked = st.checkbox(f"{label}（+{pt}P）", key=f"flat35s_{gname}_{label}")
                    if checked:
                        total_points += pt
...
c1, c2 = st.columns(2)
with c1:
    var_rate = st.number_input("変動金利 初期値（％）", value=0.894, step=0.001, format="%.3f")
with c2:
    flat_base = st.number_input("フラット35 基準金利（％）", value=2.820, step=0.001, format="%.3f")
...
with c3:
    property_price = st.number_input("物件価格（万円）", value=5000, step=100, min_value=0)
with c4:
    misc_cost = st.number_input("諸費用（万円）", value=200, step=10, min_value=0)
with c5:
    own_funds = st.number_input("自己資金（万円）", value=500, step=50, min_value=0)

# ──────────────────────────────────────────────────────────────────────
# ④ 借入金額・頭金率 自動計算
# ──────────────────────────────────────────────────────────────────────
loan_amount = property_price + misc_cost - own_funds
loan_amount = max(loan_amount, 0)
...
good = {y: max(var_rate - 0.01 * (y - 1), 0.25) for y in range(1, years + 1)}
scenarios["変動 GOOD（毎年-0.01%）"] = good
...
scenarios["変動 BAD（金利上昇）"] = {y: bad_tbl.get(y, 3.25) for y in range(1, years + 1)}

# フラット35 S（ポイント反映）
flat_schedule = flat35s_schedule(flat_base, total_points, years)
scenarios[f"フラット35 S（{total_points}P）"] = flat_schedule
...
# ──────────────────────────────────────────────────────────────────────
# フラット35 S ポイント金利引下げ説明
# ──────────────────────────────────────────────────────────────────────
st.markdown("---")
st.markdown("**フラット35 S 金利引下げ対応表（参考）**")
disc_table_data = {
    "合計ポイント": ["1P", "2P", "3P", "4P", "5P", "6P", "7P", "8P", "9P以上"],
    "当初5年": ["▲0.25%", "▲0.50%", "▲0.75%", "▲1.00%", "▲0.25%", "▲0.50%", "▲0.75%", "▲1.00%", "▲0.25%"],
    "6〜10年目": ["±0%", "±0%", "±0%", "±0%", "▲0.50%", "▲1.00%", "▲1.00%", "▲1.00%", "▲0.50%"],
    "11〜15年目": ["±0%"] * 8 + ["▲1.00%"],
}
st.dataframe(pd.DataFrame(disc_table_data), use_container_width=True, hide_index=True)
...
with c9:
    ...
    st.metric("固定 月額返済", f"{monthly_flat:,.0f} 円")
with c10:
...
with c11:
    invest_rate = st.number_input("想定利回り（年率 ％）", value=4.0, step=0.1, format="%.1f")
with c12:
    invest_years = st.number_input("運用期間（年）", value=int(years), step=1, min_value=1, max_value=50)

future_value = compound_investment(diff_monthly, invest_rate, invest_years)
st.success(f"差額（月 {diff_monthly:,.0f}円）を年利 {invest_rate:.1f}% で {invest_years} 年間複利運用 → **{future_value/10000:,.1f} 万円**")
...
<tr style="background:#1a4f8a; color:white;">
  ...
  <th style="padding:6px 10px; width:42.5%;">変動金利</th>
  <th style="padding:6px 10px; width:42.5%;">フラット35（固定）</th>
</tr>
<tr style="background:#eef3fb;">
  <td style="padding:6px 10px; font-weight:bold;">メリット</td>
  <td style="padding:6px 10px;">・借入時の金利が低く月々返済を抑えやすい<br>・金利低下時に返済負担が軽減<br>・団信の選択肢が多い</td>
  <td style="padding:6px 10px;">・返済額固定で家計設計が安定<br>・金利上昇リスクなし<br>・団信加入が任意（健康不安でも利用可）</td>
</tr>
<tr>
  <td style="padding:6px 10px; font-weight:bold;">デメリット</td>
  <td style="padding:6px 10px;">・金利上昇で返済額増のリスク<br>・125%ルールによる未払利息リスク<br>・家計設計が不安定</td>
  <td style="padding:6px 10px;">・初期金利が変動より高い<br>・市場金利低下の恩恵なし<br>・借入上限8,000万円<br>・適合証明（フラット35適合）が必要</td>
</tr>
</table>
</div>
"""
...
if st.button("📄 PDF を生成してダウンロード"):
    with st.spinner("PDF を生成中..."):
        pdf_bytes = build_pdf(
            principal_man=loan_amount,
            years=years,
            loan_amount_man=loan_amount,
            jishikin_man=own_funds,
