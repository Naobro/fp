...
def load_prefs(client_id: str | None) -> Dict[str, Any]:
    ...
    return {
        ...
        "labels_spec": {},
        "labels_mgmt": {},
        # 重要度：inspection を追加（未設定なら spec と同重みにする）
        "importance": {"price":1, "location":2, "size_layout":3, "spec":4, "management":5, "inspection":4}
    }
...
def to_weights(importance: Dict[str,int]) -> Dict[str,float]:
    # inspection を許容。未設定なら spec と同じ重みを与える
    raw = {
        ...
        "inspection":  imp_to_weight(importance.get("inspection", importance.get("spec",3))),
    }
    s = sum(raw.values()) or 1.0
    return {k: v/s for k,v in raw.items()}

def to_fit_score(blocks: Dict[str,float], weights: Dict[str,float]) -> float:
    # blocks にあるキーのみ重みづけ合算
...
# 内見チェックリスト 定義
# =========================
INSPECTION_GROUPS_COMMON = {
    "外装・外構": [
        ("外壁のひび割れ", "crack_wall"),
        ("外壁の色あせ", "fade_wall"),
        ("シーリングの劣化/割れ", "seal_joint"),
        ("バルコニー防水（ひび/膨れ）", "balcony_waterproof"),
        ("越境の疑い（樹木・庇等）", "encroachment_suspect"),
        ("境界標の確認可否", "boundary_marker"),
        ("眺望/前建の圧迫感", "view_block"),
    ],
    "内装": [
        ("床の傾き感（ビー玉転がり等）", "floor_slope"),
        ("建具の建付け（開閉/立て付け）", "door_fit"),
        ("壁・天井の雨染み/はがれ", "stain_ceiling"),
        ("結露/カビ臭", "condensation_smell"),
        ("家事動線（キッチン⇄洗面）", "work_flow"),
        ("収納量・動線の邪魔にならない", "storage_flow"),
        ("家具レイアウトの容易さ", "furniture_layout"),
        ("冷蔵庫/洗濯機/カーテン寸法の適合", "appliance_curtain_fit"),
    ],
    "設備": [
        ("水栓/排水の漏れ・滲み", "water_leak"),
        ("換気扇/24H換気の動作", "ventilation_work"),
        ("給湯器 立上り・異音", "water_heater_run"),
        ("エアコン有無・動作", "aircon_work"),
        ("分電盤の容量/ブレーカー余り", "breaker_capacity"),
        ("ネット方式（光配線/他）", "net_type"),
    ],
}

INSPECTION_GROUPS_MANSION = {
    "管理・共用部": [
        ("共用部の清掃状態", "common_clean"),
        ("ゴミ置場の清潔さ/分別", "trash_station"),
        ("駐輪場の混雑", "bicycle_crowd"),
        ("駐車場出入りのしやすさ", "parking_ease"),
        ("掲示板の雰囲気（トラブル/注意書き）", "board_mood"),
    ]
}

INSPECTION_GROUPS_HOUSE_ONLY = {
    "外装・外構（戸建て追加）": [
        ("屋根の傷み/浮き（目視）", "roof_damage"),
        ("雨樋の破損/詰まり", "gutter_damage"),
        ("基礎（ひび/欠け/鉄筋露出）", "foundation_crack"),
        ("擁壁/高基礎の劣化・不同沈下兆候", "retaining_bad"),
        ("床下点検口の有無（見えれば）", "underfloor_access"),
    ]
}

INSPECTION_RATING_CHOICES = ["良い","普通","悪い","不明"]  # 1.0/0.7/0.3/0.6

def inspection_choice_to_score(v: str) -> float:
    if v == "良い": return 1.0
    if v == "普通": return 0.7
    if v == "悪い": return 0.3
    return 0.6  # 不明
...
CRITICAL_NEG_FLAGS = {
    ...
    "stain_ceiling",     # 雨染み
    "floor_slope",       # 大きな傾き
    "retaining_bad",     # 擁壁/高基礎の不良
    "foundation_crack",  # 基礎の深刻なひび
}

def score_inspection_block(p: Dict[str,Any]) -> float:
    ins: Dict[str, Any] = p.get("inspection", {})
    ratings: Dict[str, str] = ins.get("ratings", {})
    if not ratings:
        return 0.6  # 未入力なら中立寄り

    # 基本平均
    vals = [inspection_choice_to_score(v) for v in ratings.values()]
    base = sum(vals) / max(1, len(vals))

    # 重要項目の重み微調整
    for key in ["water_leak","stain_ceiling","retaining_bad","foundation_crack","balcony_waterproof"]:
        if key in ratings:
            s = inspection_choice_to_score(ratings[key])
            base = (base*len(vals) + s) / (len(vals)+1)

    # クリティカルの強い減点（どれかが「悪い」なら全体0.55倍）
    for k in CRITICAL_NEG_FLAGS:
        if ratings.get(k) == "悪い":
            base *= 0.55
            break
...
for i, p in enumerate(props):
    ...
    # 内見データの器
    p.setdefault("inspection", {"ratings": {}, "note": ""})
...
with st.container(border=True):
    ...
    with b3:
        if st.button("🗑 クリア（このページ）", use_container_width=True):
            st.session_state.props = [{"name": f"物件{i+1}","type":"マンション","price_man":0.0,"year_built":0,"area_m2":0.0,"kanri":0,"shuzen":0,"inspection":{"ratings":{},"note":""}} for i in range(5)]
...
for i, tab in enumerate(tabs):
    with tab:
        ...
        # ===== 内見チェックリスト（現地確認） =====
        st.subheader("内見チェックリスト（現地確認）")
        ins = p.setdefault("inspection", {"ratings": {}, "note": ""})
        ratings: Dict[str,str] = ins.setdefault("ratings", {})

        # 共通
        with st.container(border=True):
            for group, items in INSPECTION_GROUPS_COMMON.items():
                with st.expander(group, expanded=False):
                    cols = st.columns(2)
                    for j, (label, keyname) in enumerate(items):
                        col = cols[j % 2]
                        current = ratings.get(keyname, "不明")
                        ratings[keyname] = col.radio(label, INSPECTION_RATING_CHOICES, index=INSPECTION_RATING_CHOICES.index(current) if current in INSPECTION_RATING_CHOICES else 3, key=f"ins_c_{i}_{keyname}", horizontal=True)

        # 種別別
        if p["type"] == "マンション":
            with st.container(border=True):
                for group, items in INSPECTION_GROUPS_MANSION.items():
                    with st.expander(group, expanded=False):
                        cols = st.columns(2)
                        for j, (label, keyname) in enumerate(items):
                            col = cols[j % 2]
                            current = ratings.get(keyname, "不明")
                            ratings[keyname] = col.radio(label, INSPECTION_RATING_CHOICES, index=INSPECTION_RATING_CHOICES.index(current) if current in INSPECTION_RATING_CHOICES else 3, key=f"ins_m_{i}_{keyname}", horizontal=True)
        else:
            with st.container(border=True):
                for group, items in INSPECTION_GROUPS_HOUSE_ONLY.items():
                    with st.expander(group, expanded=False):
                        cols = st.columns(2)
                        for j, (label, keyname) in enumerate(items):
                            col = cols[j % 2]
                            current = ratings.get(keyname, "不明")
                            ratings[keyname] = col.radio(label, INSPECTION_RATING_CHOICES, index=INSPECTION_RATING_CHOICES.index(current) if current in INSPECTION_RATING_CHOICES else 3, key=f"ins_h_{i}_{keyname}", horizontal=True)

        ins["note"] = st.text_area("メモ（要補修箇所/指摘事項）", value=ins.get("note",""), key=f"ins_note_{i}", height=80)
...
for p in props:
    ...
    # 内見ブロック（共通）
    b_insp = score_inspection_block(p)
    ...
    blocks = {"price":b_price,"location":b_loc,"size_layout":b_size,"spec":b_spec,"management":b_mgmt,"inspection":b_insp}
    fit     = to_fit_score(blocks, to_weights(prefs.get("importance", {})))
